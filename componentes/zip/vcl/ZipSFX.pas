(*
  ...
  @author((C)2003-2004 markus stephany, delphizip@mirkes.de, all rights reserved.)
...
uses
  Windows, Messages, SysUtils, Classes, Graphics, SFXInterface, ZipMsg,
  DZUtils, SFXStructs;
  ...
  protected
    ...
    *)
    property OverwriteMode: TSFXOverwriteMode
      Read FOverwriteMode Write FOverwriteMode default somAsk;
    ...
    *)
    property DefaultExtractPath: String Read FDefaultExtractPath
      Write FDefaultExtractPath;
    (* This string will be stored in the created self extracting archive (SFX).<br>
    ...
    *)
    ...
    (* This string will be stored in the created self extracting archive (SFX).<br>
       ...
       - This string must not exceed 255 characters.<br><br><br><br>
    *)
    property FallbackExtractPath: String Read FFallbackExtractPath
      Write FFallbackExtractPath;
    ...
    *)
    property HowToDelete: THowToDeleteFile Read FHowToDelete
...
begin
  ...
  FSFXLanguage := AnsiUpperCase(DZSFX_LANG);
  // set at compile time when using make -DLANG=<lg> vcl
...
begin
  ...
  if (fiSrc in [fiEmpty, fiOther]) or (fiTrg in [fiEmpty, fiOther]) then
    raise EDelphiZip.CreateResDisp(GE_NoZipSpecified, True);

  if (fiSrc = fiZip) and (fiTrg = fiExe) then
    ConvertToSFX
  else if (fiSrc = fiExe) and (fiTrg = fiZip) then
    ConvertToZip
  else
    // not yet localized
...
begin
  ...
  if not FileExists(FSourceFile) and (( not bCreateNew) or (soDetached in Options)) then
    raise EDelphiZip.CreateResDisp(GE_NoZipSpecified, True);
...
begin
  ...
  hMod := LoadLibraryEx(PChar(sFile), 0, DONT_RESOLVE_DLL_REFERENCES or
    LOAD_LIBRARY_AS_DATAFILE);
  if hMod <> 0 then
    try
      FillChar(szBuf, sizeof(szBuf), #0);
      if LoadString(hMod, 1024 (* ID of the dzsfx.bin identifier string *),
        szBuf, sizeof(szBuf)) <> 0 then
      begin
...
begin
  ...
  begin
    ...
    with _INCLUDED_LANGUAGES do
      if IndexOfName(FSFXLanguage) > -1 then
...
var
  hRes, hFindSFX, hFindSize: THandle;
  pData:   Pointer;
  cSize:   Cardinal;
...
begin
  ...
  if hFindSFX <> 0 then
  begin
    hFindSize := FindResource(HInstance, PChar('DZSFX' + SLanguage + '_BIN_SIZE'),
      RT_RCDATA);
    if (hFindSize <> 0) and (SizeOfResource(HInstance, hFindSize) >=
  ...
  begin
    ...
    if (FSFXPath = '') or (FSFXPath[length(FSFXPath)] = '\') then
      FSFXPath := FSFXPath + 'DZSFX' + SLanguage + '.BIN';
    (*RP  end of change  *)
    ...
    if not FileExists(FSFXPath) then
      raise EDelphiZip.CreateResDrive(SF_NoZipSFXBin, 'DZSFX' + SLanguage + '.BIN');  // RP
...
constructor TSFXHeaderStream.CreateFromZipStream(str: TZipArchiveStream;
  oZipSFX: TCustomZipSFX);
begin
  inherited Create;
  ...
  begin
    ...
    if not GetNewHeaderProperties then
...
begin
  ...
  try
    ...
    if (FStream.Read(recDEnd, sizeof(recDEnd)) = sizeof(recDEnd)) and
      ...
      FStream.Seek( -((sizeof(recDEnd) * 2) + (recDEnd.NameLen + recDEnd.ExtLen)),
        soFromCurrent);
    ...
    begin
      ...
      begin
        ...
        if FStream.Read(FDialogTitle[1], CaptionSize) <> CaptionSize then
      ...
      begin
        ...
        if FStream.Read(FDefaultExtractPath[1], PathSize) <> PathSize then
          raise EDelphiZip.CreateResDisp(CZ_SFXTypeUnknown, True);
      end
      else
        FDefaultExtractPath := '';
      ...
      begin
        ...
        if FStream.Read(FCommandLine[1], CmdLineSize) <> CmdLineSize then
          raise EDelphiZip.CreateResDisp(CZ_SFXTypeUnknown, True);
      end
      else
        FCommandLine := '';
      ...
      begin
        ...
        if FStream.Read(FFallbackExtractPath[1], RegFailPathSize) <>
          RegFailPathSize then
          raise EDelphiZip.CreateResDisp(CZ_SFXTypeUnknown, True);
      end
      else
      ...
      begin
        ...
        if FStream.Read(FMessage[1], StartMsgSize) <> StartMsgSize then
          raise EDelphiZip.CreateResDisp(CZ_SFXTypeUnknown, True);
      end
      else
        FMessage := '';
...
begin
  ...
  try
    ...
    try
      ...
      if FStream.Read(pData^, iMax) <> iMax then
        raise EDelphiZip.CreateResDisp(DS_NoValidZip, True);

      for i := 0 to iMax - 3 do
        if (pData[i] = 'M') and (pData[i + 1] = 'P') and
          ((pData[i + 2] = 'U') or (pData[i + 2] = 'V')) then
          with FZipSFX do
          begin
            Result   := True;
            ...
            if (bFlags and 1) > 0 then
              Include(FOptions, soAskCmdLine);
            if (bFlags and 2) > 0 then
              Include(FOptions, soAskFiles);
            if (bFlags and 4) > 0 then
              Include(FOptions, soHideOverWriteBox);
            if (bFlags and 8) > 0 then
              FOverwriteMode := somOverwrite;
            if (bFlags and 16) > 0 then
              FOverwriteMode := somSkip;
            //if not (bFlags and 32 > 0) then SFXOptions := SFXOptions + [SFXCheckSize];
            if (bFlags and 64) > 0 then
              Include(FOptions, soAutoRun);

            if pData[i + 2] = 'U' then
            begin
              SetString(FDialogTitle, pData + i + 7, Integer(pData[i + 4]));
              SetString(FDefaultExtractPath, pData + i + Integer(
                pData[i + 4]) + 7, Integer(pData[i + 5]));
              SetString(FCommandLine, pData + i + Integer(
                pData[i + 4]) + Integer(pData[i + 5]) + 7, Integer(pData[i + 6]));
              FMessage := '';
              FFallbackExtractPath := '';
            end
            else begin
              if (bFlags and 128) > 0 then
                Include(FOptions, soNoSuccessMsg);
              pLoop := pData + i + 8;
...
procedure TSFXHeaderStream.WriteDetachedHeader(str: TStream; const FileName: String);
var
...
var
  iPosOFEOC, iNewSize, iDelta, iSize, iPos: Integer;
begin
...
begin
  ...
  begin
    ...
    begin
      Result := Count;
      if Result > (sizeof(FEOC) - FPosition) then
...
// enum sfx resources
function ResEnumFunc(hMod: THandle; szType, szName: PChar;
  lParam: TStrings): BOOL; stdcall;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            // get locale info
            SetString(sr1, szBuf, GetLocaleInfo(MakeLCID(pw^, SORT_DEFAULT),
              LOCALE_SNATIVELANGNAME, szBuf, sizeof(szBuf)));
            UnlockResource(h2);
          end;
          FreeResource(h2);
        end;
